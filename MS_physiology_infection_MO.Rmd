---
title: "R code for 'Apparent physiological costs of early-life blood parasite infection in a vertebrate host'"
...
df <- 
  ...
  mutate(Infection_intensity = as.factor(Infection_intensity)) %>% 
  mutate(Prevalence = as.factor(Prevalence)) %>% ## Prevalence at time point 2 ???
  ...
  mutate(Wing_cm = as.numeric(Wing)) %>% 
  ...
  mutate(Growth_rate = as.numeric(Growth_rate_unlog)) %>% 
...
## -----------------------------------------------------------------------------
post_treatment <- with(df, table(Prevalence, Treatment))
...
mean_before_after <- data.frame(
  ...
  Infection_intensity = c(as.character(df$Infection_intensity.1), as.character(df$Infection_intensity)),
  Treatment = 
    c(as.character(df$Treatment[df$Sampling_rank %in% 2]), as.character(df$Treatment[df$Sampling_rank %in% 2])),
  Timepoint = 
    c(rep("Pre-treatment", times = length(df$Treatment)), rep("Post-treatment", times = length(df$Infection_intensity)))) %>% 
  mutate(Treatment = as.factor(Treatment)) %>% 
...
tab_model(
  ...
  file = "./Physiological_parameters_in_decreasing_control_treated.html")
```
...
```{r mixed model breathing rate, warning=F, message=F}
## Run model
## ----------------------------------------------------------------------------- 
lmer_br <- lmer(
...
```{r mixed model body temp after, warning=FALSE, message=F}
## Model 
## -----------------------------------------------------------------------------
lmer_bt = lmer(
...
```{r mixed model body condition, warning=FALSE, message=F}
## Model 
## -----------------------------------------------------------------------------
lmer_bc <- lmer(
...
```{r mixed model body temp, warning=FALSE, message=F}
# Model 
...
sjPlot::tab_model(
  ...
  df.method = "kr",
  dv.labels = c("Breathing rate","Body temperature","Δ Body condition","Growth rate"),
  file = "Table_results_lmer_physiological_parameters_p.val_kr.html")
```
...
```{r mixed model breathing rate 2, warning=F, message=F}
## Model 
## -----------------------------------------------------------------------------
lmer_br_para = lmer(
  Breathing_rate ~ Infection_intensity + Infection_intensity:Treatment + days_resample + 
...
m1 = table(df$Infection_intensity[!is.na(df$Breathing_rate)])
...
lmer_bt_para = lmer(
  body_temperature ~ Infection_intensity + Infection_intensity:Treatment + days_resample +
...
mod_bt_para = get_model_data(lmer_bt_para, 
                             type = 'pred',
                             terms = "Infection_intensity") %>% 
   mutate(x = factor(c("0","1","2","3","4"), levels = c("0","1","2","3","4")))

# Table with sample sizes specific for body temperature change data

m2 = table(df$Infection_intensity[!is.na(df$body_temperature)])
...
```{r mixed model body condition change 2, warning=FALSE, message=F, echo=T}
# Model 
lmer_bc_para <- lmer(
  delta_body_condition ~ Infection_intensity + Infection_intensity:Treatment + days_resample +
...
m3 = table(df$Infection_intensity[!is.na(df$delta_body_condition)])
...
```{r mixed model growth rate 2, warning=FALSE, message=F, echo=T}
# Model 
lmer_gr_para = lmer(
  Growth_rate ~ Infection_intensity + Infection_intensity:Treatment +
...
m4 = table(df$Infection_intensity[!is.na(df$Growth_rate)])
...
sjPlot::tab_model(lmer_br_para,lmer_bt_para,lmer_bc_para,lmer_gr_para,
                  ...
                  file = "table_results_parasitemia_lmer_physiological_parameters_p.val_kr.html")
...
tolres <- ggplot(data = df[!df$Infection_intensity %in% c(NA,0) & !df$Diff_Inf_Class %in% NA,],
                aes(y = delta_body_condition, x = as.numeric(Infection_intensity), col = Diff_Inf_Class)) + 
...
gg5 = ggplot(df[!df$Infection_intensity %in% NA,]) + 
...
gg6 <- ggplot(df[!df$Infection_intensity %in% NA,]) + 
...
gg7 = ggplot(df[!df$Infection_intensity %in% NA,]) + 
...
gg8 = ggplot(df[!df$Infection_intensity %in% NA,]) + 
