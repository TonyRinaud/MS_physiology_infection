...
sjPlot::tab_model(
  ...
  dv.labels = c(bquote("Breathing rate at T"[1]),
                ...
                "\U0394 Body condition",
                "Growth rate"),
  file = "./Tables/Table1_results_lmer_physiological_parameters_p.val_kr_Treatment.html")
```
...
ggsave(fig4,filename = "./Figs/Fig4_predicted_physio_param_infection_intensities_Treatment.png",units = "px", width = 3700, height = 3700)
...
sjPlot::tab_model(
  ...
  dv.labels = c(bquote("Breathing rate at T"[1]),
                ...
                "\U0394 Body condition",
                "Growth rate"),
  file = "./Tables/Table2_results_parasitemia_lmer_physiological_parameters_p.val_kr_Treatment.html")
```
...
ggsave(figS1,filename = "./Figs/FigS1_predicted_physio_param_infection_stages_Treatment.png",units = "px", width = 3700, height = 3700)
...
Fig_S3C = ggplot(df[!df$Infection_intensity.2 %in% NA,]) + 
  ...
  geom_hline(yintercept = mean(df$delta_body_condition, na.rm = T), linetype = "dashed") + 
  labs(x = bquote("Parasitemia at T"[1]), y = "\U0394 Body condition") + 
  geom_jitter(color = "lightgrey", size = 2, width = .3) + 
  stat_summary(fun = mean, colour = "cornflowerblue", geom = "errorbar", 
               size = 2.8 , width = 0.05, show.legend = FALSE, 
               fun.max = function(x) mean(x) + sd(x),
