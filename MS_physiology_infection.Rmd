...
perm.t.test(as.numeric(as.character(df$Infection_intensity.1))~df$Treatment)

## Testing for Treatment effect on parasitemia change between first and second sampling
## -----------------------------------------------------------------------------

lmerTest::lmer(intensity_change~Treatment+Year+days_resample+(1|Nest), data=df[!df$intensity_change %in% -100,]) %>%
  summary()
```
