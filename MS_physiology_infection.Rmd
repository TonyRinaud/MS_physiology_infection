...
treatment_meaninf_changes=ggplot(mean_infint)+
  aes(Sampling_rank,Infection_intensity,group=Treatment, fill=Treatment)+
  ...
  # ylim(c(0,3))+
  geom_point(aes(colour=Treatment),position=position_jitterdodge(jitter.width = 0.2, jitter.height = 0.5))+  # position=position_jitterdodge()
  ...
  stat_summary(geom="errorbar", width=0.04, size=2)+
...
# tolres_deltamod=lm(body_condition.1~Infection_intensity.1*Diff_Inf_Class, data=tol_resdata)
# 
...
# 
# tolres_delta_first=ggplot(data =data2016_2020[! data2016_2020$Infection_intensity.1 %in% NA & ! data2016_2020$Diff_Inf_Class %in% c(NA,"Uninfected"),], aes(y=body_condition.1,x=as.numeric(Infection_intensity.1),col=Diff_Inf_Class))+
...
tolres_deltamod=lm(body_condition.2~Infection_intensity*Diff_Inf_Class, data=tol_resdata)
...
gg1=ggplot(data2016_2020[!data2016_2020$Diff_Inf_Class %in% NA,])+
  ...
  labs(x="", y="Breathing rate (z-transformed)")+
...
gg2=ggplot(data2016_2020[!data2016_2020$Diff_Inf_Class %in% NA,])+
  ...
  labs(x="", y="Body temperature (z-transformed)")+
  geom_jitter(color ="lightgrey", size=2, width=.3)+
  stat_summary(fun=mean, colour="cornflowerblue", geom="errorbar", 
               size=2.8 , width=0.05, show.legend = FALSE, 
               fun.max = function(x) mean(x) + sd(x),
...
gg3=ggplot(data2016_2020[!data2016_2020$Diff_Inf_Class %in% NA,])+
  ...
  labs(x="", y="Body condition (z-transformed)")+
  geom_jitter(color ="lightgrey", size=2, width=.3)+
  stat_summary(fun=mean, colour="cornflowerblue", geom="errorbar", 
               size=2.8 , width=0.05, show.legend = FALSE, 
               fun.max = function(x) mean(x) + sd(x),
...
gg4=ggplot(data2016_2020[!data2016_2020$Diff_Inf_Class %in% NA,])+
  ...
  labs(x="", y="Growth rate (z-transformed)")+
  geom_jitter(color ="lightgrey", size=2, width=.3)+
  stat_summary(fun=mean, colour="cornflowerblue", geom="errorbar", 
               size=2.8 , width=0.05, show.legend = FALSE, 
               fun.max = function(x) mean(x) + sd(x),
