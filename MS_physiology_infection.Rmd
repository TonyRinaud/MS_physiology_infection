...
Fig_5B = ggplot(mod_bt_para) + 
  ...
  geom_segment(size = .8,
               aes(x = 1, y = max(mod_bt_para$higher_CI, na.rm = T) + 0.2,
                   xend = 1, yend = max(mod_bt_para$higher_CI, na.rm = T) + 0.05)) + 
  geom_segment(size = .8,
               aes(x = 5, y = max(mod_bt_para$higher_CI, na.rm = T) + 0.05,
                   xend = 5, yend = max(mod_bt_para$higher_CI, na.rm = T) + 0.2))
```
...
Fig_5D = ggplot(mod_gr_para) + 
  ...
  theme(legend.position = "none", 
        ...
        plot.title = element_text(hjust = 0.5, face = "bold")) + 
  geom_segment(size = .8,
               aes(x = 1, y = 4, xend = 5, yend = 4)) + 
  geom_text(aes(x = 3, y = 5 - 0.63),
            label = "p = 0.094", size = 5) + 
  geom_segment(size = .8,
               aes(x = 1, y = 4, xend = 1, yend = 4 + 0.5)) + 
  geom_segment(size = .8,
               aes(x = 5, y = 4, xend = 5, yend = 4 + 0.5))
```

### Summary four models

```{r summary}
## Create multipanel Figure 5
## -----------------------------------------------------------------------------
fig5 = Fig_5A + Fig_5B + Fig_5C + Fig_5D + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 16, face = "bold"))
fig5
...
## ----------------------------------------------------------------------------- 
Fig_S1A = ggplot(df[!df$Diff_Inf_Class %in% NA,]) + 
...
Fig_S1B = ggplot(df[!df$Diff_Inf_Class %in% NA,]) + 
...
Fig_S1C = ggplot(df[!df$Diff_Inf_Class %in% NA,]) + 
...
Fig_S1D = ggplot(df[!df$Diff_Inf_Class %in% NA,]) + 
...
## ----------------------------------------------------------------------------- 
Fig_S2A = ggplot(df[!df$Infection_intensity.2 %in% NA,]) + 
...
Fig_S2B <- ggplot(df[!df$Infection_intensity.2 %in% NA,]) + 
...
Fig_S2C = ggplot(df[!df$Infection_intensity.2 %in% NA,]) + 
...
Fig_S2D = ggplot(df[!df$Infection_intensity.2 %in% NA,]) + 
